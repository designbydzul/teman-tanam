          ...
          <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 'var(--app-max-width)', zIndex: 5001 }}>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
                <h2 style={{ fontFamily: "'Caveat', cursive", fontSize: '1.75rem', fontWeight: 600, color: '#2D5016', margin: 0 }}>Catat Pemupukan</h2>
              ...
              {alreadyFertilizedToday.length > 0 && (
                <div style={{ backgroundColor: '#FEF3C7', border: '1px solid #FF9800', borderRadius: '12px', padding: '12px 16px', marginBottom: '20px' }}>
                  <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '13px', fontWeight: 600, color: '#B45309', margin: '0 0 4px 0' }}>{alreadyFertilizedToday.length} tanaman sudah dipupuk hari ini:</p>
                  <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '12px', color: '#92400E', margin: 0, lineHeight: '1.4' }}>{alreadyFertilizedToday.map(p => p.name).join(', ')}</p>
                </div>
              )}
