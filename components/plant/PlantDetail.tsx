...
const isValidUUID = (str: string | number | undefined | null): boolean => {
  if (!str || typeof str !== 'string') return false;
  const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
  return uuidRegex.test(str);
};

const PlantDetail: React.FC<PlantDetailProps> = ({ plant, onBack, onEdit, onDelete, onRecordAction, onSavePlant }) => {
  ...
  const [showWateringDrawer, setShowWateringDrawer] = useState<boolean>(false);
  const [showFertilizingDrawer, setShowFertilizingDrawer] = useState<boolean>(false);
  const [fertilizingNotes, setFertilizingNotes] = useState<string>('');
  const [fertilizingPhoto, setFertilizingPhoto] = useState<File | null>(null);
  const [fertilizingPhotoPreview, setFertilizingPhotoPreview] = useState<string | null>(null);
  const [isSubmittingAction, setIsSubmittingAction] = useState<boolean>(false);
  const fertilizingPhotoInputRef = useRef<HTMLInputElement | null>(null);

  // Pruning drawer state
  const [showPruningDrawer, setShowPruningDrawer] = useState<boolean>(false);
  const [pruningNotes, setPruningNotes] = useState<string>('');
  ...
  const [otherActionName, setOtherActionName] = useState<string>('');
  const [otherActionNotes, setOtherActionNotes] = useState<string>('');
  const [otherActionPhoto, setOtherActionPhoto] = useState<File | null>(null);
  const [otherActionPhotoPreview, setOtherActionPhotoPreview] = useState<string | null>(null);
  const otherActionPhotoInputRef = useRef<HTMLInputElement | null>(null);

  // History detail drawer state
  const [showHistoryDetailDrawer, setShowHistoryDetailDrawer] = useState<boolean>(false);
  const [selectedHistoryEntry, setSelectedHistoryEntry] = useState<TimelineEntry | null>(null);
  const [showDeleteHistoryConfirm, setShowDeleteHistoryConfirm] = useState<boolean>(false);
  ...
  const getActionSubtitle = (daysSince: number | null | undefined, frequencyDays: number | null, actionType: 'water' | 'fertilize') => {
  ...
  const formatTime = (timestamp: string | null | undefined): string | null => {
  ...
  const getActionLabel = (actionType: string, notes: string | null | undefined = ''): string => {
    switch (actionType) {
      case 'siram':
        return 'Penyiraman';
      case 'pupuk':
  ...
  const getCleanNotes = (actionType: string, notes: string | null | undefined): string | null => {
  ...
  const mapActionTypeForIcon = (actionType: string): string => {
    switch (actionType) {
      case 'siram':
        return 'water';
      case 'pupuk':
  ...
  const handleActionLog = async (actionType: string, options: ActionOptions = {}): Promise<ActionResult> => {
  ...
  const handleDeleteTodayAction = async (actionType: string): Promise<ActionResult> => {
    const plantName = plantData.customName;
  ...
  const handleFertilizingPhotoSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setFertilizingPhotoPreview(reader.result as string);
      };
  ...
  const handlePruningPhotoSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      const reader = new FileReader();
      reader.onloadend = () => {
        setPruningPhotoPreview(reader.result as string);
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  const handleOtherActionPhotoSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      const reader = new FileReader();
      reader.onloadend = () => {
        setOtherActionPhotoPreview(reader.result as string);
      };
      reader.readAsDataURL(file);
    }
  };
            ...
            >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '24px' }}>
                <motion.button
                  ...
                  onMouseEnter={(e) => !wateringStatus.doneToday && ((e.currentTarget as HTMLElement).style.backgroundColor = '#F5F5F5')}
                  onMouseLeave={(e) => !wateringStatus.doneToday && ((e.currentTarget as HTMLElement).style.backgroundColor = '#FFFFFF')}
                >
                ...
                <motion.button
                  ...
                  onMouseEnter={(e) => !fertilizingStatus.doneToday && ((e.currentTarget as HTMLElement).style.backgroundColor = '#F5F5F5')}
                  onMouseLeave={(e) => !fertilizingStatus.doneToday && ((e.currentTarget as HTMLElement).style.backgroundColor = '#FFFFFF')}
                >
              ...
              {selectedHistoryEntry.photoUrl && (
                <div style={{ marginBottom: '20px' }}>
                  <img
                    ...
                    alt="Action photo"
                    onClick={() => setTimelinePhotoPreview(selectedHistoryEntry.photoUrl ?? null)}
                    style={{
                      width: '100%',
                      borderRadius: '8px',
                      objectFit: 'cover',
