...
export async function syncLocation(item) {
  try {
    ...
    console.log('[syncLocation] Starting sync:', action, locationData);
    ...
    if (action === 'create') {
      console.log('[syncLocation] Creating location with:', {
...
export async function syncAll() {
  ...
  try {
    ...
    console.log('[syncService] Queue contents:', JSON.stringify(queue, null, 2));

    // Process items in order (important for dependencies like plant -> action)
    for (const item of queue) {
      let syncResult;

      console.log(`[syncService] Processing item:`, item.type, item.action, item.id);
      console.log(`[syncService] Item data:`, JSON.stringify(item.data, null, 2));
